...
Or, clone the repository and checkout out a tag, for example:

bc. git checkout -f api-v5.2.3
...
Note that, "since Notepad++ 8.3":8.3, all of the _former_ APIs return 64-bit values also; these are:

* @SCI_FINDTEXT@
* @SCI_FORMATRANGE@
* @SCI_GETTEXTRANGE@

Calling any of the above APIs is functionally equivalent in Notepad++ 8.4.3 or later.
However, a future version of Scintilla will "deprecate the former APIs":sciNext (the ones without the @*FULL@ suffix).
For forward compatibility, the following types are provided by 'Include/Scintilla.inc':
...
* @TNppPlugin.HasFullRangeApis@ - returns @true@ if the @SCI_FINDTEXTFULL@, @SCI_FORMATRANGEFULL@ and @SCI_GETTEXTRANGE@ APIs are available


h3. Breaking Changes in Notepad++ 8.4.3 and later

'Include/Scintilla.inc' no longer provides the @TSearchResultMarking@ type.
...
h3. Optional Scintilla Features (as of v5.2.3)[4]
...
fn4. "Scintilla 5.2.3 definitions":https://github.com/notepad-plus-plus/notepad-plus-plus/blob/ed4bb1a93e763001aac842698fcde0856ba8b0bc/scintilla/include/Scintilla.h
